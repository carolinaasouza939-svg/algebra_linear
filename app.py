...
de estabilidade de sistemas, compressão de dados (PCA), Google PageRank, entre outros.
   ...
   with aba_exemplo:
        ...
        st.markdown("Dada a matriz $A = \\begin{bmatrix} 2 & 1 \\\\ 0 & 3 \\end{bmatrix}$, vamos encontrar seus autovalores e autovetores:")
        
        st.markdown("### 1. Equação Característica")
        st.markdown("Calculamos $\\det(A - \\lambda I) = 0$:")
        st.latex(r"\det\begin{bmatrix} 2-\lambda & 1 \\ 0 & 3-\lambda \end{bmatrix} = (2-\lambda)(3-\lambda) = 0")
        st.markdown("Portanto, os autovalores são **$\\lambda_1 = 2$** e **$\\lambda_2 = 3$**.")
        
        st.markdown("### 2. Determinando os Autovetores")
        st.markdown("- **Para $\\lambda_1 = 2$:** Substituindo na matriz $(A - 2I)\\vec{v} = 0$, obtemos o autovetor:")
        st.latex(r"\vec{v}_1 = \begin{bmatrix} 1 \\ 0 \end{bmatrix}")
        st.markdown("- **Para $\\lambda_2 = 3$:** Substituindo na matriz $(A - 3I)\\vec{v} = 0$, obtemos o autovetor:")
        st.latex(r"\vec{v}_2 = \begin{bmatrix} 1 \\ 1 \end{bmatrix}")
